...
class TestUpload:
    ...
    def test_upload_invalid(self, api_client):
        ...
        assert response.status_code == 400, f'{URL} has not rejected invalid extension'
        assert Image.objects.filter(user=user).count() == 0, f'{URL} has saved an invalid image'
